...
- [ ] T001 Create Jekyll plugin file structure in `_plugins/translation_validator.rb`
- [ ] T002 [P] Create Liquid include for language toggle in `_includes/language-toggle.html`
- [ ] T003 [P] Create Liquid include for translation disclaimer in `_includes/translation-disclaimer.html`
- [ ] T004 [P] Create JS module for language preference and toast in `assets/js/language-toggle.js`

## Phase 3.2: Contract Tests First (TDD) ⚠️ MUST COMPLETE BEFORE 3.3
**CRITICAL: These validation checks MUST be written and MUST pass/fail correctly before implementation**
- [ ] T005 [P] Validator log format contract test - create `_plugins/test_translation_validator.rb` to verify `LANG-TX | LEVEL | CODE | message` format
- [ ] T006 [P] Toggle behavior contract test - create manual test script `tools/test-toggle-behavior.rb` to verify data attributes
- [ ] T007 [P] Disclaimer placement contract test - create manual test script `tools/test-disclaimer-placement.rb` to verify DOM insertion
- [ ] T008 [P] Integration test for missing translation toast - add test scenarios to `quickstart.md` manual verification
- [ ] T009 [P] Integration test for session language preference - add test scenarios to `quickstart.md` manual verification

## Phase 3.3: Core Implementation (ONLY after tests are written)
- [ ] T010 Translation validator plugin core - PostVariant indexing and validation in `_plugins/translation_validator.rb`
- [ ] T011 Validator logging system - implement log format contract in `_plugins/translation_validator.rb`
- [ ] T012 Language toggle Liquid include - render available translations and data attributes in `_includes/language-toggle.html`
- [ ] T013 Translation disclaimer Liquid include - conditional rendering logic in `_includes/translation-disclaimer.html`
- [ ] T014 Language preference JS module - sessionStorage handling in `assets/js/language-toggle.js`
- [ ] T015 Toast notification JS module - accessible aria-live implementation in `assets/js/language-toggle.js`

## Phase 3.4: Layout Integration
- [ ] T016 Integrate language toggle into post layout - modify `_layouts/post.html`
- [ ] T017 Integrate translation disclaimer into post layout - modify `_layouts/post.html`
- [ ] T018 Filter homepage by language - modify `_layouts/home.html` for Chinese-only root
- [ ] T019 Update English homepage filtering - ensure `/en/` shows English posts in `_layouts/home.html`
- [ ] T020 Integrate session preference persistence across site navigation

## Phase 3.5: Data Flow & Validation
- [ ] T021 Implement duplicate variant resolution (FR-030) - keep latest modified file
- [ ] T022 Implement missing origin validation (FR-029) - warn when English lacks Chinese
- [ ] T023 Implement original_slug requirement (FR-022) - error on missing field
- [ ] T024 Implement disclaimer trigger logic (FR-026-029) - detect translated posts
- [ ] T025 Implement 302 redirect for non-existent translation URLs (FR-020) - redirect to available variant + toast
 - [ ] T025A Enforce `page_id == original_slug` for all posts (Polyglot integration) - inject if absent (supports permalink_lang)
 - [ ] T025B Generate toggle link set using `page.permalink_lang` map when available (fallback to validator map)
 - [ ] T025C Override Polyglot fallback listing: ensure no default-language fallback content leaks into non-origin language lists (FR-019, FR-025)
 - [ ] T025D Insert SEO `<link rel="alternate" hreflang>` tags using `page.permalink_lang` (enhancement)

## Phase 3.6: Polish & Testing
- [ ] T026 [P] Manual verification checklist (FR-009, FR-018, FR-019, FR-025) - homepage filtering behavior
- [ ] T027 [P] Manual verification checklist (FR-003, FR-018) - toggle navigation between counterparts
- [ ] T028 [P] Manual verification checklist (FR-004, FR-021, FR-031) - toast accessibility behavior
- [ ] T029 [P] Manual verification checklist (FR-005, FR-012) - session preference persistence & storage fallback
- [ ] T030 [P] Manual verification checklist (FR-026–FR-029) - translation disclaimer placement & variants
- [ ] T031 Create cleanup task for temporary relative_url wrappers removal (FR-024)
...
- [ ] T044 [P] Raw template leakage grep test (FR-006) - search for `{#` or Liquid artifacts in `_site`
 - [ ] T045 [P] permalink_lang mapping integrity test (Polyglot) - confirm all pages expose consistent language map
 - [ ] T046 [P] hreflang tags presence & correctness test - ensure each variant outputs full set
 - [ ] T047 [P] Fallback suppression test - verify no unintended default-language content appears in secondary language lists
